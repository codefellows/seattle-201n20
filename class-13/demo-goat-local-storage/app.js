...
// IF the data exists, we "grab it off the shelf"
var retrievedResults = localStorage.getItem('goatResults');

// IF the data exists, we use it!
// NOTE:  we need to NOT reinstantiate goats IF Local Storage ALREADY EXISTS
if (retrievedResults) {
  // "unpack the data" - convert from JSON to JavaScript
  var parsedRetrievedResults = JSON.parse(retrievedResults);
  // console.log(parsedRetrievedResults);
  // IF data exists we assign that data to the goats array so we can add more data to existing data
  goats = parsedRetrievedResults;
} else {
  //executable code
...
function populateRenderQueue() {
  while (renderQueue.length > 3) {
    renderQueue.shift(); //shift, unshift, pop, and push - what do those do?  STRONG RECOMMEND TO PLAY in repl
  }
...
// local storage experimentation --not used for functionality
// var storedNumber = 42;
// localStorage.setItem('thisIsMyNumber', storedNumber);

// var noResultGoats = goats;
// console.log(noResultGoats);
// localStorage.setItem('goatsWrong', noResultGoats);
// var stringifiedGoats = JSON.stringify(noResultGoats);
// // console.log(stringifiedGoats);
// localStorage.setItem('noReultGoatsStringified', stringifiedGoats);

// var retrievedGoats = localStorage.getItem('noReultGoatsStringified');
// // console.log(retrievedGoats);
// var parsedNoResultGoats = JSON.parse(retrievedGoats);
// console.log(parsedNoResultGoats);
...
function handleClick(event) {
  ...
  if (clicks === totalClicksAllowed) {
    ...
    // "packs data away" to be stored, we convert to JSON
    var stringifiedResults = JSON.stringify(goats);

    // store the data - "put the box on the shelf"
    localStorage.setItem('goatResults', stringifiedResults);
